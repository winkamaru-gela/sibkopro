...
import { 
    ...
    FileText, Download // Import Icon Baru
} from 'lucide-react';
...
const PrintPreviewModal = ({ isOpen, onClose, title, children }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] bg-slate-900/95 flex flex-col animate-in fade-in backdrop-blur-sm print:bg-white print:static">
            ...
            <div className="bg-slate-800 text-white px-4 py-3 shadow-md flex justify-between items-center print-header-actions">
                <div>
                    <h2 className="text-lg font-bold flex items-center gap-2"><Printer size={20}/> Pratinjau Cetak</h2>
                    <p className="text-xs text-slate-400">Pastikan opsi privasi sudah sesuai sebelum mencetak.</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={onClose} className="px-4 py-2 text-sm font-bold text-slate-300 hover:bg-slate-700 rounded-lg">Tutup</button>
                    <button onClick={() => window.print()} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-lg text-sm font-bold shadow-lg">Cetak Sekarang</button>
                </div>
            </div>
            
            <div className="flex-1 overflow-y-auto p-4 flex justify-center bg-slate-900/50 print:bg-white print:p-0 print:overflow-visible print-portal-root">
                <div className="print-paper-content bg-white text-slate-900 shadow-2xl w-full max-w-[210mm] min-h-[297mm] p-10 origin-top h-fit mx-auto">
                    {children}
                </div>
            </div>
        </div>, document.body
...
const StudentPointBook = ({ students, pointLogs, journals, settings, sanctionRules }) => {
    ...
    const pointData = useMemo(() => {
        ...
        // Net Score Asli (Violation - Achievement)
    ...
    const handleDownloadWord = () => {
        ...
        // Logic Warna Teks untuk Word (Menggunakan Hex Color)
        ...
        let htmlContent = `
            ...
            <body style="font-family: Arial, sans-serif; font-size: 12px;">
                ...
                <div style="text-align: center; margin-bottom: 20px; border-bottom: 3px double black; padding-bottom: 10px;">
                    <h3 style="margin:0; text-transform: uppercase;">${settings?.government || ''}</h3>
                    <h3 style="margin:0; text-transform: uppercase;">${settings?.department || ''}</h3>
                    <h2 style="margin:5px 0; text-transform: uppercase;">${settings?.name || 'NAMA SEKOLAH'}</h2>
                    <p style="margin:0; font-style: italic;">${settings?.address || ''}</p>
                </div>
                ...
                <table style="${tableStyle}">
                    ...
                    <tbody>
                        <tr>
                            <td style="${tdStyle} ${centerTd} color: red; font-weight: bold; font-size: 14px;">${violationTotal}</td>
                            <td style="${tdStyle} ${centerTd} color: green; font-weight: bold; font-size: 14px;">${achievementTotal}</td>
                            <td style="${tdStyle} ${centerTd} color: ${scoreColor}; font-weight: bold; font-size: 14px;">${displayScore}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto items-center">
                    ...
                    <div className="relative w-full md:w-80" ref={searchRef}>
                        <div className="relative">
                            ...
                            {selectedStudentId ? (
                                <button 
                                    onClick={() => { setSelectedStudentId(''); setSearchQuery(''); setIsSearchOpen(true); }}
                    ...
                    <div className="flex gap-2 w-full md:w-auto">
                        {/* TOMBOL DOWNLOAD DOC (BARU) */}
                        <button 
                            disabled={!selectedStudentId}
                            onClick={handleDownloadWord}
                            className="bg-green-600 hover:bg-green-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-4 py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 shadow-sm transition-all flex-1 md:flex-none"
                        ...
                        <button 
                            ...
                            className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-4 py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 shadow-sm transition-all flex-1 md:flex-none"
                        >
                            ...
                            <div className="flex gap-4">
                                <label className="flex items-center gap-2 cursor-pointer select-none bg-white px-3 py-2 rounded-lg border border-yellow-100 shadow-sm">
                                    <input 
                                        ...
                                        checked={privacyMode.maskDescription}
                                        onChange={e => setPrivacyMode({...privacyMode, maskDescription: e.target.checked})}
                                    />
                                    <span className="text-xs font-bold text-slate-700">Samarkan Topik Masalah</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer select-none bg-white px-3 py-2 rounded-lg border border-yellow-100 shadow-sm">
                                    <input 
                                        type="checkbox" 
                                        className="w-4 h-4 accent-red-600"
                                        checked={privacyMode.hideCounseling}
                                        onChange={e => setPrivacyMode({...privacyMode, hideCounseling: e.target.checked})}
                                    />
                                    <span className="text-xs font-bold text-slate-700">Sembunyikan Riwayat BK</span>
                                </label>
                            </div>
                        </div>
        ...
        <div className={`text-slate-900 ${isPrintVersion ? 'text-sm' : 'p-8'}`}>
            ...
            <div className="flex items-center justify-between border-b-4 border-double border-black pb-4 mb-6">
                ...
                <div className="flex-1 text-center px-4">
                    <h4 className="font-bold text-sm uppercase">{settings?.government}</h4>
                    <h3 className="font-bold text-base uppercase">{settings?.department}</h3>
                    <h1 className="font-extrabold text-xl uppercase my-1">{settings?.name}</h1>
            ...
            <table className="w-full border-collapse border border-black mb-6 text-center text-sm">
                ...
                <tbody>
                    <tr>
                        <td className="border border-black p-3 font-bold text-red-600 text-lg">{violationTotal}</td>
                        <td className="border border-black p-3 font-bold text-green-600 text-lg">{achievementTotal}</td>
                        <td className={`border border-black p-3 font-bold text-lg ${scoreColorClass}`}>
                            {displayScore}
                        </td>
                    </tr>
                </tbody>
            ...
            {/* 5. RIWAYAT LAYANAN (PRIVACY AWARE) */}
            {!privacyMode.hideCounseling && (
                <div className="mb-8">
