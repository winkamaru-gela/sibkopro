...
import { 
    ...
    Gavel, AlertOctagon, UserCheck, ChevronUp, UserCog, UserMinus, Hash, Download
} from 'lucide-react';
...
const StudentManager = ({ students, journals, pointLogs, sanctionRules, onAdd, onEdit, onDelete, onImport, onMoveClass }) => {
    ...
    const [viewDetail, setViewDetail] = useState(null); // State untuk Popup Buku Pribadi
    ...
    // --- 3. LOGIKA HITUNGAN POIN (Safe Access) ---
    ...
    const handleEditClick = useCallback((student) => {
        ...
        setFormData({ 
            ...
            homeroomTeacherNip: student.homeroomTeacherNip || ''
        });
        setShowForm(true);
        setTimeout(() => {
            if (formRef.current) {
    ...
    const handleExportData = () => {
        ...
        const dataToExport = filteredStudents.map(s => ({
            "NISN": s.nisn, "Nama Lengkap": s.name, "Kelas": s.class, "L/P": s.gender,
            "Tempat Lahir": s.pob, "Tanggal Lahir (DD-MM-YYYY)": s.dob, "Alamat": s.address, "No HP Siswa": s.phone,
            "Nama Ayah": s.fatherName, "Pekerjaan Ayah": s.fatherJob, "No HP Ayah": s.fatherPhone, "Status Ayah (Hidup/Meninggal)": s.fatherDeceased ? "Meninggal" : "Hidup",
            "Nama Ibu": s.motherName, "Pekerjaan Ibu": s.motherJob, "No HP Ibu": s.motherPhone, "Status Ibu (Hidup/Meninggal)": s.motherDeceased ? "Meninggal" : "Hidup",
            "Nama Wali (Opsional)": s.parent, "Pekerjaan Wali (Opsional)": s.jobParent, "No HP Wali (Opsional)": s.parentPhone,
            "Resiko (LOW/MEDIUM/HIGH)": s.riskLevel, "Wali Kelas": s.homeroomTeacher, "NIP Wali Kelas": s.homeroomTeacherNip, "Guru Wali": s.guardianTeacher
        }));
        ...
        ws['!cols'] = wscols;
        const fileName = filterClass ? `Data_Siswa_Kelas_${filterClass.replace(/[^a-zA-Z0-9]/g, '_')}.xlsx` : `Data_Seluruh_Siswa.xlsx`;
        XLSX.writeFile(wb, fileName);
    };

    const handleDownloadTemplate = () => {
        const templateData = [{
            "NISN": "12345678", "Nama Lengkap": "Budi Santoso", "Kelas": "X-1", "L/P": "L",
            "Tempat Lahir": "Jakarta", "Tanggal Lahir (DD-MM-YYYY)": "06-08-2008", "Alamat": "Jl. Merdeka No 1", "No HP Siswa": "08123456789",
            "Nama Ayah": "Bpk. Santoso", "Pekerjaan Ayah": "Wiraswasta", "No HP Ayah": "0811...", "Status Ayah (Hidup/Meninggal)": "Hidup",
            "Nama Ibu": "Ibu Siti", "Pekerjaan Ibu": "IRT", "No HP Ibu": "0812...", "Status Ibu (Hidup/Meninggal)": "Hidup",
            "Nama Wali (Opsional)": "", "Pekerjaan Wali (Opsional)": "", "No HP Wali (Opsional)": "",
            "Resiko (LOW/MEDIUM/HIGH)": "LOW", "Wali Kelas": "Ibu Guru A", "NIP Wali Kelas": "1980xxxx", "Guru Wali": "Bpk Guru B"
        }];
        const ws = XLSX.utils.json_to_sheet(templateData);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Template Siswa");
        const wscols = Array(22).fill({wch:20});
        ws['!cols'] = wscols;
        XLSX.writeFile(wb, "Template_Data_Siswa_Lengkap_V2.xlsx");
    };
                    ...
                    if (row["Nama Lengkap"]) {
                        ...
                        if (!finalParent) {
                            if (getVal("Nama Ayah") && !fatherDead) {
                                finalParent = getVal("Nama Ayah"); finalJob = getVal("Pekerjaan Ayah"); finalPhone = getVal("No HP Ayah");
                            } else if (getVal("Nama Ibu") && !motherDead) {
                                finalParent = getVal("Nama Ibu"); finalJob = getVal("Pekerjaan Ibu"); finalPhone = getVal("No HP Ibu");
                            }
                        }
                        newStudents.push({
                            nisn: getVal("NISN"), name: getVal("Nama Lengkap"), class: getVal("Kelas"),
                            gender: getVal("L/P").toUpperCase() === 'P' ? 'P' : 'L', pob: getVal("Tempat Lahir"), dob: parsedDob, 
                            address: getVal("Alamat"), phone: getVal("No HP Siswa"),
                            fatherName: getVal("Nama Ayah"), fatherJob: getVal("Pekerjaan Ayah"), fatherPhone: getVal("No HP Ayah"), fatherDeceased: fatherDead,
                            motherName: getVal("Nama Ibu"), motherJob: getVal("Pekerjaan Ibu"), motherPhone: getVal("No HP Ibu"), motherDeceased: motherDead,
                            parent: finalParent, jobParent: finalJob, parentPhone: finalPhone,
                            riskLevel: ['LOW', 'MEDIUM', 'HIGH'].includes(getVal("Resiko (LOW/MEDIUM/HIGH)")) ? getVal("Resiko (LOW/MEDIUM/HIGH)") : 'LOW',
                            homeroomTeacher: getVal("Wali Kelas"), homeroomTeacherNip: getVal("NIP Wali Kelas"), guardianTeacher: getVal("Guru Wali")
                        });
                    }
                });
                if (newStudents.length > 0 && confirm(`Ditemukan ${newStudents.length} data siswa. Import?`)) onImport(newStudents);
    ...
    // --- 7. RENDERERS (Memoized) ---
    
    const RenderedMobileList = useMemo(() => (
            ...
            {filteredStudents.map(s => (
                <div key={s.id} className={`bg-white p-4 rounded-xl shadow-sm border relative overflow-hidden transition-all active:scale-[0.99] ${selectedIds.includes(s.id) ? 'border-orange-400 bg-orange-50' : 'border-slate-200'}`}>
                    ...
                    <div className="flex gap-3 items-start mb-3" onClick={() => { setViewDetail(s); setExpandBio(false); }}>
                        <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 flex-shrink-0"><User size={24}/></div>
                        <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-slate-800 text-base line-clamp-1">{s.name}</h3>
                            <div className="flex items-center gap-2 text-xs mt-1">
                                <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-bold">{s.class}</span><span className="text-slate-500">{s.nisn || 'No NISN'}</span>
                            </div>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 bg-slate-50 p-2.5 rounded-lg mb-3">
                        <div className="flex items-center gap-1.5"><Phone size={12}/> {s.phone || '-'}</div><div className="flex items-center gap-1.5"><User size={12}/> {s.parent || 'Wali -'}</div>
                    </div>
                    <div className="flex items-center justify-between">
                        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${RISK_LEVELS[s.riskLevel].badge}`}>{RISK_LEVELS[s.riskLevel].label}</span>
                        <div className="flex gap-2">
                            <button onClick={() => { setViewDetail(s); setExpandBio(false); }} className="p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 border border-slate-200"><Eye size={16}/></button>
                            <button onClick={() => handleEditClick(s)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 border border-blue-100"><Edit size={16}/></button>
                            <button onClick={() => onDelete(s.id)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 border border-red-100"><Trash2 size={16}/></button>
                        </div>
                    </div>
                </div>
            ))}
        </div>
    ), [filteredStudents, isMoveMode, selectedIds, handleSelectOne, handleEditClick, onDelete]);

    const RenderedDesktopTable = useMemo(() => (
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 text-slate-600 uppercase text-xs font-bold tracking-wider border-b border-slate-200">
                        <tr>
                            {isMoveMode && <th className="p-4 w-10 text-center"><input type="checkbox" onChange={handleSelectAll} checked={selectedIds.length > 0 && selectedIds.length === filteredStudents.length} className="w-4 h-4 accent-orange-500 cursor-pointer"/></th>}
                            <th className="p-4">Identitas Siswa</th><th className="p-4">Pendamping</th><th className="p-4">Kontak</th><th className="p-4">Status Resiko</th><th className="p-4 text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {filteredStudents.map(s => (
                            <tr key={s.id} className={`hover:bg-slate-50 transition-colors group ${selectedIds.includes(s.id) ? 'bg-orange-50' : ''}`}>
                                {isMoveMode && <td className="p-4 text-center"><input type="checkbox" checked={selectedIds.includes(s.id)} onChange={() => handleSelectOne(s.id)} className="w-4 h-4 accent-orange-500 cursor-pointer"/></td>}
                                <td className="p-4">
                                    <div className="font-bold text-slate-700 text-base">{s.name}</div>
                                    <div className="text-xs text-slate-500 mt-0.5 flex gap-2"><span className="bg-slate-200 px-1.5 rounded font-mono text-slate-700 font-bold">{s.class}</span><span>{s.nisn || '-'}</span><span className="bg-slate-100 px-1.5 rounded font-bold">{s.gender}</span></div>
                                </td>
                                <td className="p-4">
                                    <div className="text-xs text-slate-600 flex flex-col gap-1">
                                        <span title="Wali Kelas" className="flex items-center gap-1 font-medium"><UserCog size={12} className="text-blue-500"/> {s.homeroomTeacher || '-'}</span>
                                        {s.homeroomTeacherNip && <span title="NIP Wali Kelas" className="flex items-center gap-1 pl-4 text-[10px] text-slate-400 font-mono"><Hash size={10}/> {s.homeroomTeacherNip}</span>}
                                        <span title="Guru Wali" className="flex items-center gap-1 mt-1"><Users size={12} className="text-green-500"/> {s.guardianTeacher || '-'}</span>
                                    </div>
                                </td>
                                <td className="p-4">
                                ...
                                <td className="p-4"><span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide text-white ${RISK_LEVELS[s.riskLevel].badge}`}>{RISK_LEVELS[s.riskLevel].label}</span></td>
                                <td className="p-4 text-center">
                                    <div className="flex justify-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => { setViewDetail(s); setExpandBio(false); }} className="text-blue-600 bg-blue-50 hover:bg-blue-100 p-2 rounded-lg transition-colors border border-blue-100" title="Lihat Profil"><Eye size={16}/></button>
                                        <button onClick={() => handleEditClick(s)} className="text-orange-600 bg-orange-50 hover:bg-orange-100 p-2 rounded-lg transition-colors border border-orange-100" title="Edit Data"><Edit size={16}/></button>
                                        <button onClick={() => onDelete(s.id)} className="text-red-600 bg-red-50 hover:bg-red-100 p-2 rounded-lg transition-colors border border-red-100" title="Hapus Data"><Trash2 size={16}/></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
                        {filteredStudents.length === 0 && <tr><td colSpan={isMoveMode ? 6 : 5} className="p-10 text-center text-slate-400 italic">Data siswa tidak ditemukan.</td></tr>}
                    </tbody>
                </table>
            </div>
        </div>
    ), [filteredStudents, isMoveMode, selectedIds, handleSelectAll, handleSelectOne, handleEditClick, onDelete]);

    return (
        <div className="p-4 md:p-6 space-y-6 relative min-h-screen pb-20">
            {/* HEADER TOOLBAR */}
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col lg:flex-row justify-between items-center gap-4 sticky top-0 z-10">
                ...
                <div className="flex flex-col md:flex-row gap-2 w-full lg:w-auto items-center">
                    {isMoveMode ? (
                        <div className="flex flex-wrap items-center gap-2 bg-orange-50 p-2 rounded-lg border border-orange-200 animate-in fade-in w-full md:w-auto">
                            ...
                            <input className="flex-1 text-sm p-2 border rounded-md focus:ring-2 ring-orange-400 outline-none min-w-[120px]" placeholder="Kelas Tujuan..." value={targetClass} onChange={e => setTargetClass(e.target.value.toUpperCase())}/>
                            <button onClick={executeMoveClass} className="bg-orange-600 text-white p-2 rounded-md hover:bg-orange-700 text-xs font-bold px-3">PINDAH</button>
                        ...
                        <>
                            ...
                            <div className="relative">
                                <select 
                                    className="w-full md:w-36 pl-3 pr-8 py-2.5 border rounded-lg text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none appearance-none cursor-pointer font-medium text-slate-600"
                            ...
                            <div className="relative w-full md:w-56">
                                ...
                                <input className="w-full pl-10 p-2.5 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-slate-50 focus:bg-white transition-colors" placeholder="Cari nama, NISN..." value={search} onChange={e=>setSearch(e.target.value)}/>
                            </div>
                            ...
                            <div className="hidden md:flex gap-2">
                                ...
                                {/* TOMBOL EXPORT */}
                                <button onClick={handleExportData} className="bg-green-100 text-green-700 hover:bg-green-200 px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors" title="Export Data Tampil"><Download size={18}/></button>
                                
                                <button onClick={handleDownloadTemplate} className="bg-slate-100 text-slate-700 hover:bg-slate-200 px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors" title="Template Excel"><FileSpreadsheet size={18}/></button>
                                ...
                                <button onClick={handleAddClick} className="bg-blue-600 text-white hover:bg-blue-700 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow-sm transition-colors"><PlusCircle size={18}/> {showForm ? 'Batal' : 'Tambah'}</button>
                            </div>
                        </>
                    )}
                </div>
            ...
            {showMobileMenu && !isMoveMode && (
                <div className="md:hidden bg-white p-3 rounded-xl shadow-lg border border-slate-200 grid grid-cols-2 gap-3 animate-in slide-in-from-top-2">
                    <button onClick={() => {handleAddClick(); setShowMobileMenu(false);}} className="col-span-2 bg-blue-600 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2"><PlusCircle size={20}/> {showForm ? 'Batal Input' : 'Tambah Siswa Baru'}</button>
                    ...
                    <button onClick={handleDownloadTemplate} className="flex flex-col items-center gap-1 p-3 bg-slate-50 text-slate-700 rounded-lg text-xs font-bold border border-slate-200"><FileSpreadsheet size={20}/> Template</button>
            ...
            {showForm && (
                <div ref={formRef} className="bg-white p-6 rounded-xl border border-blue-200 shadow-xl animate-in fade-in slide-in-from-top-4 relative overflow-hidden scroll-mt-20">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">{editingId ? <Edit className="text-blue-600"/> : <PlusCircle className="text-blue-600"/>}{editingId ? 'Edit Data Siswa' : 'Input Siswa Baru'}</h3>
                        <button onClick={resetForm} className="text-slate-400 hover:text-red-500"><X size={24}/></button>
                    </div>
                    <form onSubmit={(e) => { e.preventDefault(); if(editingId) onEdit({...formData, id: editingId}); else onAdd(formData); resetForm(); }} className="space-y-6">
                        {/* FORM FIELDS */}
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-end gap-4 pt-2 border-t">
                            ...
                            <div className="flex gap-3 w-full md:w-auto">
                                <button type="button" onClick={resetForm} className="flex-1 md:flex-none px-6 py-2.5 text-slate-500 hover:bg-slate-100 rounded-lg font-bold text-sm">Batal</button>
            ...
            {RenderedMobileList}
            {RenderedDesktopTable}

            {/* MODAL VIEW DETAIL (POPUP BUKU PRIBADI) */}
            {viewDetail && (
                    ...
                    <div className="bg-white md:rounded-2xl shadow-2xl w-full max-w-4xl h-full md:max-h-[95vh] flex flex-col overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto bg-slate-100 pb-10">
                            {(() => {
                                ...
                                const isDeficit = netScore > 0;
                                const isSurplus = netScore < 0;

                                return (
                                    <>
                                        <div className="bg-white p-4 md:p-6 border-b border-slate-200 sticky top-0 z-20 shadow-sm flex items-center gap-4">
                                            <div className="w-14 h-14 bg-gradient-to-br from-blue-100 to-blue-50 rounded-full flex items-center justify-center text-blue-600 font-bold text-xl flex-shrink-0 shadow-inner">{viewDetail.name.charAt(0)}</div>
                                            <div className="flex-1 min-w-0">
                                                <h2 className="text-xl font-bold text-slate-800 line-clamp-1">{viewDetail.name}</h2>
                                                <div className="flex items-center gap-2 text-sm text-slate-500 mt-0.5">
                                                    <span className="bg-slate-100 px-2 rounded font-mono font-bold text-slate-700 border border-slate-200">{viewDetail.class}</span><span>{viewDetail.nisn}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="p-4 md:p-6 space-y-4">
                                            {activeSanction ? (
                                                <div className="bg-red-600 text-white rounded-xl p-5 shadow-lg flex flex-col md:flex-row items-start md:items-center gap-4 border-l-8 border-red-800">
                                                    <div className="bg-red-800/50 p-3 rounded-full flex-shrink-0"><AlertOctagon size={32}/></div>
                                                    <div className="flex-1"><p className="text-xs font-bold text-red-200 uppercase tracking-widest mb-1">STATUS: PERLU TINDAK LANJUT</p><h3 className="text-2xl font-bold leading-tight">{activeSanction.action}</h3><p className="text-red-100 mt-1 opacity-90">Sanksi: {activeSanction.penalty}</p></div>
                                                    <div className="text-center bg-white/10 px-4 py-2 rounded-lg"><span className="block text-xs font-bold opacity-70">Total Poin</span><span className="block text-2xl font-bold">{Math.abs(netScore)}</span></div>
                                                </div>
                                            ) : (
                                                <div className="bg-green-600 text-white rounded-xl p-4 shadow-lg flex items-center gap-4 border-l-8 border-green-800">
                                                    <div className="bg-green-800/50 p-2 rounded-full"><CheckCircle size={24}/></div>
                                                    <div><h3 className="font-bold text-lg">Status Disiplin: Aman</h3><p className="text-xs text-green-100 opacity-80">Siswa belum mencapai batas poin sanksi.</p></div>
                                                </div>
                                            )}
                                            <div className="grid grid-cols-3 gap-2 md:gap-4">
                                                <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm text-center"><p className="text-[10px] md:text-xs text-red-500 font-bold uppercase mb-1">Pelanggaran</p><p className="text-xl md:text-2xl font-bold text-slate-800">{violationTotal}</p></div>
                                                <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm text-center"><p className="text-[10px] md:text-xs text-green-600 font-bold uppercase mb-1">Prestasi</p><p className="text-xl md:text-2xl font-bold text-slate-800">{achievementTotal}</p></div>
                                                <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm text-center relative overflow-hidden">
                                                    ...
                                                    <p className="text-[10px] md:text-xs text-slate-400 font-bold uppercase mb-1">Poin Bersih</p><p className={`text-xl md:text-2xl font-bold ${isDeficit ? 'text-red-600' : isSurplus ? 'text-green-600' : 'text-slate-800'}`}>{Math.abs(netScore)}</p>
                                                </div>
                                            </div>
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                                    <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center"><h4 className="font-bold text-slate-700 flex items-center gap-2"><Gavel size={16}/> Catatan Poin</h4><span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full text-slate-600">{logs.length}</span></div>
                                                        ...
                                                        {logs.length > 0 ? logs.map(log => (
                                                            <div key={log.id} className="p-4 hover:bg-slate-50 transition-colors flex gap-3">
                                                                ...
                                                                <div className="flex-1">
                                                                    <div className="flex justify-between items-start"><span className={`text-[10px] font-bold uppercase tracking-wider ${log.type === 'violation' ? 'text-red-600' : 'text-green-600'}`}>{log.type === 'violation' ? 'Pelanggaran' : 'Prestasi'}</span><span className="text-[10px] text-slate-400">{formatIndoDate(log.date)}</span></div>
                                                                    <p className="text-sm font-medium text-slate-800 mt-0.5">{log.description}</p><p className="text-xs text-slate-500 mt-1 font-mono">Nilai: {log.value}</p>
                                                                </div>
                                                            </div>
                                                ...
                                                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                                    <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center"><h4 className="font-bold text-slate-700 flex items-center gap-2"><Activity size={16}/> Layanan BK</h4><span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full text-slate-600">{history.length}</span></div>
                                                    <div className="max-h-[300px] overflow-y-auto divide-y divide-slate-100">
                                                        {history.length > 0 ? history.map(h => (
                                                            <div key={h.id} className="p-4 hover:bg-slate-50 transition-colors">
                                                                <div className="flex justify-between items-center mb-1"><span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase">{h.serviceType}</span><span className="text-[10px] text-slate-400">{formatIndoDate(h.date)}</span></div>
                                            ...
                                            <div className="bg-white rounded-xl border border-slate-200 shadow-sm mt-6">
                                                <button onClick={() => setExpandBio(!expandBio)} className="w-full p-4 flex justify-between items-center bg-slate-50 hover:bg-slate-100 transition-colors rounded-t-xl"><span className="font-bold text-slate-700 flex items-center gap-2"><UserCheck size={16}/> Informasi Lengkap Siswa</span>{expandBio ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}</button>
                                                {expandBio && (
                                                    <div className="p-5 border-t border-slate-200 animate-in slide-in-from-top-2 fade-in">
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                                                            ...
                                                            <div className="space-y-3">
                                                                ...
                                                                <div className="bg-slate-50 p-2 rounded text-xs space-y-1"><div className="font-bold text-slate-600">Ayah</div><div className="flex justify-between"><span>Nama:</span> <span className="font-medium">{viewDetail.fatherName || '-'} {viewDetail.fatherDeceased && '(Alm)'}</span></div><div className="flex justify-between"><span>Pekerjaan:</span> <span className="font-medium">{viewDetail.fatherJob || '-'}</span></div><div className="flex justify-between"><span>HP:</span> <span className="font-medium">{viewDetail.fatherPhone || '-'}</span></div></div>
                                                                <div className="bg-slate-50 p-2 rounded text-xs space-y-1"><div className="font-bold text-slate-600">Ibu</div><div className="flex justify-between"><span>Nama:</span> <span className="font-medium">{viewDetail.motherName || '-'} {viewDetail.motherDeceased && '(Almh)'}</span></div><div className="flex justify-between"><span>Pekerjaan:</span> <span className="font-medium">{viewDetail.motherJob || '-'}</span></div><div className="flex justify-between"><span>HP:</span> <span className="font-medium">{viewDetail.motherPhone || '-'}</span></div></div>
                                                                <div className="bg-blue-50 p-2 rounded text-xs space-y-1 border border-blue-100"><div className="font-bold text-blue-700">Wali Utama</div><div className="flex justify-between"><span>Nama:</span> <span className="font-bold">{viewDetail.parent || '-'}</span></div><div className="flex justify-between"><span>Hubungi:</span> <span className="font-bold text-blue-600">{viewDetail.parentPhone || '-'}</span></div></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                        ...
                        <div className="p-4 border-t bg-white flex justify-end flex-shrink-0">
                            <button onClick={() => setViewDetail(null)} className="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-colors w-full md:w-auto">Tutup Buku</button>
                        </div>
                    </div>
                </div>
            )}
