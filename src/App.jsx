...
export default function App() {
    ...
    const handleLogin = (u, p) => {
        ...
        setTimeout(() => {
            ...
            if (user) setAppUser(user); else setLoginError('Salah username/password');
            setLoginLoading(false);
        }, 800);
    };

    const handleLogout = () => { if(confirm("Keluar?")) setAppUser(null); };

    // --- WRAPPER CRUD (Sama seperti sebelumnya) ---
            ...
            <Routes>
                ...
                /* --- RUTE GURU --- */
                    <>
                        ...
                        {/* Layanan BK Submenus */}
                        <Route path="/points" element={<PointManager students={students} pointLogs={pointLogs} masterPoints={masterPoints} sanctionRules={sanctionRules} onAddPoint={(d) => addDoc(collection(db, 'point_logs'), {...d, teacherId: appUser.id})} onDeletePoint={(id) => deleteDoc(doc(db, 'point_logs', id))} />} />
                        ...
                        {/* Redirect semua halaman tak dikenal ke Dashboard */}
                        <Route path="*" element={<Navigate to="/" replace />} />
                    </>
                )}
            </Routes>
